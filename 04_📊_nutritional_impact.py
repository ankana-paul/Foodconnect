...
# Get real nutritional data from meal plan
    ...
    for day, meals in plan["days"].items():
        if "nutritional_info" in meals:
            ...
            if isinstance(nutrients, dict):
                nutrients["day"] = day
                nutrients["is_mock"] = False  # Flag to identify real data
...
# Get data - try real first, then mock, then combine
...
def calculate_daily_requirements(user):
    ...
    return {
        ...
        "sugar": 50    # General recommendation
    }
...
# Summary card with smart insights
...
with st.container():
    ...
    # Add visual distinction between real and mock data in charts
    ...
    if chart_type == "Bar":
        # Bar chart showing nutrient comparison by day
        ...
        # Show separate pie charts for real and mock data if both exist
        ...
        # Line chart showing nutrient trends over days
        if 'is_mock' in nutrition_df.columns:
        ...
        # Radar chart comparing nutrients to daily requirements
...
# Feature 1: Nutrient density analysis
...
# Feature 2: Meal timing analysis
with st.expander("⏰ Meal Timing Analysis"):
    ...
    # Simulate meal timing data (in a real app, this would come from user input)
    timing_data = {
